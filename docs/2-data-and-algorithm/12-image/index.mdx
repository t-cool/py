...
`ita` ライブラリで画像を扱う場合は、次のようなコードをはじめの方に書いておく必要があります。これに関しては、おまじないだと思ってください。
...
\left\lbrace
  \begin{aligned}
    R&=s\lbrace(1-t)R_1+tR_2\rbrace+(1-s)\lbrace(1-t)R_3+tR_4\rbrace \\\\
    G&=s\lbrace(1-t)G_1+tG_2\rbrace+(1-s)\lbrace(1-t)G_3+tG_4\rbrace \\\\
    B&=s\lbrace(1-t)B_1+tB_2\rbrace+(1-s)\lbrace(1-t)B_3+tB_4\rbrace
  \end{aligned}
\right.
$$
